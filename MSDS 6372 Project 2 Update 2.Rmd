...
```{r read file}
setwd("/Users/indy/Documents/MSDS/MSDS6372/project2")
...
```{r age retired}
retired = bank_imp %>% filter(job=='retired')
ggplot(retired, aes(x = y, y = age)) + geom_boxplot() + labs(title = "Participants by age", subtitle = "bandwidth = 1")
...
## bin jobs that later show to have little value
tmp<-as.character(bank_imp$job)
jobs_to_bin <-!(bank_imp$job%in% c("retired", "housemaid", "entrepreneur"))
tmp[jobs_to_bin]<-"other"
bank_imp$job_bin <-factor(tmp)

## bin all months but may since most data is in may suggesting other months are different
bank_imp$month_bin = bank_imp$month
bank_imp = bank_imp %>% mutate(month_bin = factor(if_else(month_bin == "may", "0", "1"), levels = c("0", "1")))

## most pdays are 999, lets bin instead
bank_imp$pyesno = bank_imp$pdays
bank_imp = bank_imp %>% mutate(pyesno = factor(if_else(pyesno == "999", "0", "1"), levels = c("0", "1")))

## make campaign binary high/low
bank_imp$campaign_bin = bank_imp$campaign
bank_imp = bank_imp %>% mutate(campaign_bin = factor(if_else(campaign_bin %in% c("0","1","2","3","4","5","6"), "0", "1"), levels = c("0", "1")))
...
```{r reg}
subset = c(2:4,6:9,11,13,15:21)
train_subset = train_down[,subset]
...
bank_reg <- glm(formula = y ~ ., family = binomial, data = train_subset)
summary(bank_reg)
...
bank_reg_pred <- predict(bank_reg, test_subset, type = "response")
...
```{r reg v2 complex}
subset = c(2:4,6:9,11,13,15:21,26)
train_subset = train_down[,subset]
test_subset = test[,subset]
...
``` {r LDA}
subset = c(1,11:14,16:21)
train_subset = train_down[,subset]
test_subset = test[,subset]

# Here's our linear regression formula. The variables are based on our intution looking at the correlation plots
bank_lda <- lda(formula = y ~ ., data = train_subset)
bank_lda_pred<-predict(bank_lda,test_subset)$class


confusionMatrix(bank_lda_pred, test_subset$y)
